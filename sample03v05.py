...
class ThreatSimulator:
    ...
    def add_launchd_persistence(self):
        try:
            ...
            # Use sys.argv[0] to get the script path, with fallback
            try:
                script_path = os.path.abspath(sys.argv[0])
            except NameError:
                # Fallback if __file__ or sys.argv[0] is not available
                script_path = os.path.abspath(os.path.join(os.path.dirname(__file__), sys.argv[0]) if __file__ else "threat_simulator.py")
...
<dict>
    ...
    <true/>
            ...
            print(f"LaunchAgent criado e carregado: {plist_path}")
